<template>
  <div>
    <v-card elevation="5">
      ...
      >
        <v-col cols="3">
          <div class="student" style="background-color: pink">
            <v-icon
              class="icon"
              v-show="showIcon"
              @click="deleteBtn(item.stuId)"
              >$close</v-icon
            >
            <v-img
              :src="item.image"
        ...
        >
          <el-form class="form">
            ...
            <el-row>
              ...
              <el-col span="24" v-else>
                <el-form-item label="毕业成绩：" label-width="100px">
                  <el-input v-model="editedItem.score"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
...
export default {
  ...
  methods: {
    ...
    add() {
      this.$router.push("/admin/exeStudentAdd");
    },
    deleteFn() {
    ...
    deleteBtn(stuId) {
      this.$confirm("确定删除该学生?", "提示", {
        ...
        .then(() => {
          deleteStudentInfo(stuId).then((result) => {
    ...
    sure(editedItem) {
      this.$confirm("确定修改此学生信息?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          updateStudentInfo(editedItem).then((result) => {
            showSuccessMsg(result.msg);
            this.getStudentInfoList();
          });
